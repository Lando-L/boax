...
def halton_uniform(minval: Array = jnp.zeros((1,)), maxval: Array = jnp.ones((1,))) -> Sampler:
  ...
  if out_shape[0] < 1 or out_shape[0] > functions.quasi_random.MAX_DIMENSION:
    raise ValueError(
      'Dimension must be between 1 and {}. Supplied {}'.format(
        functions.quasi_random.MAX_DIMENSION,
        out_shape[0]
      )
    )
...
def halton_normal(loc: Array = jnp.zeros((1,)), scale: Array = jnp.ones((1,))) -> Sampler:
