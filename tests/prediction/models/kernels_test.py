...
class KernelsTest(parameterized.TestCase):
  ...
  def test_rbf_function(self, key: PRNGKey, length_scale: float, num_index_points: int):
  ...
  def test_matern_one_half_function(self, key: PRNGKey, length_scale: float, num_index_points: int):
  ...
  def test_matern_one_half_kernel(self, length_scale: Array, num_index_points: int):
    x, y = jnp.ones((2, num_index_points, 2))
  ...
  def test_matern_three_halves_function(self, key: PRNGKey, length_scale: float, num_index_points: int):
    x, y = random.uniform(key, (2, num_index_points), minval=-1.0, maxval=1.0)
  ...
  def test_matern_three_halves_kernel(self, length_scale: Array, num_index_points: int):
    x, y = jnp.ones((2, num_index_points, 2))
  ...
  def test_matern_five_halves_function(self, key: PRNGKey, length_scale: float, num_index_points: int):
    x, y = random.uniform(key, (2, num_index_points), minval=-1.0, maxval=1.0)
  ...
  def test_matern_five_halves_kernel(self, length_scale: Array, num_index_points: int):
    x, y = jnp.ones((2, num_index_points, 2))
  ...
  @parameterized.parameters(
    {"key": random.key(0), "length_scale": 0.2, "period": 1.0, "variance": 0.1, "num_index_points": 10}
  )
  def test_periodic_function(self, key: PRNGKey, length_scale: float, period: float, variance: float, num_index_points: int):
    x, y = random.uniform(key, (2, num_index_points), minval=-1.0, maxval=1.0)
  ...
  @parameterized.parameters(
    {"length_scale": jnp.array([0.2, 0.5]), "period": jnp.array([1.0, 2.0]), "variance": 0.1, "num_index_points": 10}
  )
  def test_periodic_kernel(self, length_scale: Array, period: Array, variance: float, num_index_points: int):
    x, y = jnp.ones((2, num_index_points, 2))
  ...
  @parameterized.parameters(
    {"key": random.key(0), "amplitude": 5.0, "length_scale": 0.2, "num_index_points": 10},
  )
  def test_scaled(self, key: PRNGKey, amplitude: float, length_scale: float, num_index_points: int):
  ...
  @parameterized.parameters(
    {"key": random.key(0), "length_scale": 0.2, "power": 1.0, "num_index_points": 10},
  )
  def test_linear_truncated(self, key: PRNGKey, length_scale: float, power: float, num_index_points: int):
  ...
  @parameterized.parameters(
    {"key": random.key(0), "length_scales": [0.2, 0.3, 0.4], "num_index_points": 10},
  )
  def test_additive(self, key: PRNGKey, length_scales: list[int], num_index_points: int):
    x, y = random.uniform(key, shape=(2, num_index_points, 1))

    result = kernels.additive(*map(kernels.rbf, length_scales))(x, y)
    expected = sum(kernels.rbf(length_scale)(x, y) for length_scale in length_scales)
  ...
  def test_product(self, key: PRNGKey, length_scales: list[int], num_index_points: int):
    x, y = random.uniform(key, shape=(2, num_index_points, 1))

    result = kernels.product(*map(kernels.rbf, [0.2, 0.3, 0.4]))(x, y)
    expected = math.prod(
      kernels.rbf(length_scale)(x, y)
