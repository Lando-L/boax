...
from boax.core.prediction.models import means
from boax.utils.typing import Numeric, PRNGKey


class MeansTest(parameterized.TestCase):
  @parameterized.parameters({'num_index_points': 10})
  ...
  @parameterized.parameters({'x': jnp.array(2.0), 'num_index_points': 10})
  ...
  def test_linear(
    self, key: PRNGKey, scale: Numeric, bias: Numeric, num_index_points: int
  ):
