...
def upper_confidence_bound(beta: Numeric, process: Process) -> Acquisition:
    def acquisition(value: Array, **kwargs) -> Array:
        ...
        scale = jnp.vectorize(compose(jnp.sqrt, jnp.diag), signature="(k,k)->(k)")(cov)
...
def log_probability_of_improvement(best: Numeric, process: Process) -> Acquisition:
    def acquisition(value: Array, **kwargs) -> Array:
        loc, cov = process(value)
        scale = jnp.vectorize(compose(jnp.sqrt, jnp.diag), signature="(k,k)->(k)")(cov)
...
def log_expected_improvement(best: Numeric, process: Process) -> Acquisition:
    ...
    def log_ei_eps(x):
        return log1mexp(jnp.log(erfcx(_neg_inv_sqrt2 * x) * jnp.abs(x)) + _log_sqrt_pi_div_2)
    ...
    def log_ei(x):
        ...
        return jnp.where(
            ...
            scipy.stats.norm.logpdf(x) + jnp.where(x > _neg_inv_sqrt_eps, log_ei_eps(x_eps), log_ei_lower(x_lower))
        )
